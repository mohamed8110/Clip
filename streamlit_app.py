...
st.set_page_config(page_title="MNWS TikTok — 30s hard cap", layout="centered")
st.title("🎬 MNWS TikTok — vaste video + vaste audio + logo (30s)")
...
def make_cover_video(video_path, W, H, duration=30):
    """Altijd exact 'duration' seconden: subclip of loop, daarna hard cap."""
    ...
    # schalen en croppen naar 1080x1920
...
if st.button("▶️ Render 30s video"):
    try:
        ...
        logo_img = logo_img.resize((logo_width, int(logo_img.height*ratio)), _RESAMPLING)
        logo_clip= ImageClip(np.array(logo_img)).set_duration(TARGET_DURATION).set_position((24,24))
